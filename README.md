...
* Nothing, this is the beginning of the project and is, frankly speaking, a prototype for the much more polished future versions with all the added features. However, in this version there already was a base for:
* The microcontroller (in this case the Arduino UNO R4 Minima), of course, which handles all the processing and timer logic.
* This version of the project uses `delay()`-based timers and will be upgraded using `millis()`-based non-blocking timers in later versions.
...
* An ultrasonic sensor which is used to sense the distance from the object and works on simple distance formula calculations.
* A blue LED which is used to showcase the primitive states, which are SCANNING and NON_SCANNING.
...
* When an object enters the sensing range within 50 cm, the LED continuously blinks for 10 seconds to indicate that an object has been detected.
...
* An improved moving-average-based distance measurement instead of a standard direct measurement using the distance formula.
...
* An upgraded and clearly defined finite state machine (FSM) with clearly defined states (SCANNING, SENSING, TRANSITION).
* A further upgraded median filter which improves upon the moving-average filter with faster, more robust readings while also reducing the calculation overhead that was previously required.
* DRASTICALLY improved the overall readability of the code.
* VASTLY reduced the overall "jank" or dead code that existed in the previous code with well-placed and well-named constants.
* Improved the upgradeability of the project by SEVERAL folds by switching to a modular-programming-based approach.
* Fixed the bugs that were present in the previous versions, namely the improper state switching and the irregular reading of the sensor.
...
* Understanding the key difference between procedural programming and modular programming using finite state machines (FSMs), as well as using function-based programming.
* Interlacing all the various different functions and making sure the entire code block doesn't devolve into unknown states and instead remains clearly defined with objectives and goals outlined.

**NOTE: No photos or videos were shot for this version, nor were any changes made to the circuit. Hence, only the code and the README were modified.**
...
* The system works on the same basic principle that the previous version had, except now additional functionality has been added or, to be precise, allocation for the functionality has been added to the system.
* **ALL** the functions that the previous version had are now within the "BASIC" mode of the system, and that mode has been successfully declared as *functionally* complete.
* When you turn the system ON, the blue LED blinks twice (quicker than before), and then the RGB LED will blink a PURPLE colour four times.
* The system starts with the power turned OFF and must be turned on via a button.
* After the button is pressed, the RGB flashes WHITE once, followed by running the system in BASIC mode (all the functionalities of Version 2.0).
* The button pressing and the white light flashing are now fixed regardless of how many times you reset.
...
* Further improving the use of non-blocking timers and state machines, especially since the program has evolved into a complicated 600+ line program with several states WITHIN states.
...
* What's the usage of `#ifndef`, `#define`, and `#endif`, and the overall usage of `.h` files in order to store the global variables?
* How to stop oneself from needlessly adding features when they're not needed.
* How exactly does one maintain a huge program such as this with 600+ lines without constantly running into roadblocks?
* When to omit a feature when the implementation of that particular feature would hinder the overall development significantly.

## What's New In Version 2.1

* The headline feature is the creation of two separate MODES within the system called BASIC and ADVANCED.
* The new sonar-based pinging system using the buzzer during the SCANNING cycle.
* The new WHITE colour blinking from the RGB LED to indicate the power shifting from OFF to ON.
* Two different glows of the LED when the modes get switched. The two different glows being PURPLE and a multi-colour glow.
* Division of the program into several different files in order to implement proper segmentation, reduce clutter in the main program, and improve development efficiency.
